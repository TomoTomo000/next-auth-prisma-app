...
import { signUpAction, AuthState } from "@/lib/auth/action";
  ...
  useEffect(() => {
    if (state.message && !shown) {
      if (state.type === 'success') {
        ...
        snackbar.error(state.message);
      }
      setShown(true);
    } else if (!state.message) {
      setShown(false);
    }
  }, [state.message, state.type, shown, snackbar, router]);

  return (
    <div className="w-full">
      <form action={dispatch} onSubmit={() => setShown(false)}>
